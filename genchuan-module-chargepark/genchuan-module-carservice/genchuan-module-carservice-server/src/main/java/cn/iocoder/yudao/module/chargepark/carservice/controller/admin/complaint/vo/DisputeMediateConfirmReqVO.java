...
@Schema(description = "管理后台 - 纠纷调解确认 Request VO")
@Data
public class DisputeMediateConfirmReqVO {

    @Schema(description = "纠纷调解记录 ID,仅支持调解中状态", requiredMode = Schema.RequiredMode.REQUIRED, example = "1024")
    @NotNull(message = "纠纷 ID 不能为空")
    private Long id;

    @Schema(description = "调解结果", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "调解结果不能为空")
