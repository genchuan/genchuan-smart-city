 ...
 * - 支持清空缓存
 */
public class VrvCodeQueryUtils {

    // JdbcTemplate（建议通过 Spring 注入）
    private static JdbcTemplate jdbcTemplate;

    public static void setJdbcTemplate(JdbcTemplate jdbcTemplate) {
        VrvCodeQueryUtils.jdbcTemplate = jdbcTemplate;
    }
    ...
    private static String resolveTableName(Class<?> clazz) {
        if (cacheEnabled) {
            return TABLE_CACHE.computeIfAbsent(clazz, VrvCodeQueryUtils::parseTableName);
        } else {
    ...
    private static String resolveCodeField(Class<?> clazz, String manualFieldName) {
        ...
        if (cacheEnabled) {
            return FIELD_CACHE.computeIfAbsent(clazz, VrvCodeQueryUtils::parseCodeField);
        } else {
    ...
    private static String parseCodeField(Class<?> clazz) {
        ...
        for (Field field : fields) {
            if (field.isAnnotationPresent(VrvCodeField.class)) {
        ...
        if (annoFields.size() > 1) {
            throw exception("@VrvCodeField 标记字段不唯一：" + clazz.getSimpleName());
        }
