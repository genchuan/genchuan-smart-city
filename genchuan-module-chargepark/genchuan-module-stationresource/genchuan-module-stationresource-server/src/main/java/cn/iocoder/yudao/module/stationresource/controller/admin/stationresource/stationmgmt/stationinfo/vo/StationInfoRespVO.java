...
public class StationInfoRespVO {
    ...
    @ExcelProperty("[收费标准] VARCHAR(255)")
    private String feeStandard;

    @Schema(description = "[所属片区] 关联片区信息表area_info", example = "27106")
    @ExcelProperty("[所属片区] 关联片区信息表area_info")
    private Long areaId;
    ...
    @ExcelProperty("[运营类型] 如：直接管理/甲方代运营/本地化部署/横向对接/数据互通，关联芋道字典表：station_info_operate_type")
    private String operateType;

    @Schema(description = "[状态] 如：未生效/已生效/已禁用，关联芋道字典表：station_info_status", requiredMode = Schema.RequiredMode.REQUIRED, example = "2")
    @ExcelProperty("[状态] 如：未生效/已生效/已禁用，关联芋道字典表：station_info_status")
    ...
    @ExcelProperty("[设备绑定数] INT，默认0")
    private Integer deviceCount;

    @Schema(description = "[车位绑定数] INT，默认0", example = "12943")
    @ExcelProperty("[车位绑定数] INT，默认0")
