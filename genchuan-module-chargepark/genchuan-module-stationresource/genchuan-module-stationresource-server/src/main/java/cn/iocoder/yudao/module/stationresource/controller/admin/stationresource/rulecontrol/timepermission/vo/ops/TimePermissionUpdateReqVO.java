package cn.iocoder.yudao.module.stationresource.controller.admin.stationresource.rulecontrol.timepermission.vo.ops;

import io.swagger.v3.oas.annotations.media.Schema;
import jakarta.validation.constraints.NotBlank;
import jakarta.validation.constraints.NotNull;
import lombok.Data;



@Schema(description = "管理后台 - 时段权限修改 Request VO")
@Data
public class TimePermissionUpdateReqVO {

    @Schema(description = "主键ID", requiredMode = Schema.RequiredMode.REQUIRED, example = "1")
    @NotNull(message = "主键ID不能为空")
    private Long id;

    @Schema(description = "所属场站", requiredMode = Schema.RequiredMode.REQUIRED, example = "1")
    @NotNull(message = "所属场站不能为空")
    private Long stationId;

    @Schema(description = "生效时段", requiredMode = Schema.RequiredMode.REQUIRED, example = "08:00-20:00")
    @NotBlank(message = "生效时段不能为空")
    private String timeRange;

    @Schema(description = "准入权限", requiredMode = Schema.RequiredMode.REQUIRED, example = "会员车辆")
    @NotBlank(message = "准入权限不能为空")
    private String permission;

    @Schema(description = "最长停留时长 单位：分钟", example = "360")
    private Integer maxStay;

    @Schema(description = "工作日配置")
    private String workdayConfig;

    @Schema(description = "节假日配置")
    private String holidayConfig;

    @Schema(description = "高峰配置")
    private String peakConfig;

    @Schema(description = "平峰配置")
    private String offpeakConfig;

    @Schema(description = "备注")
    private String remark;

    @Schema(description = "备用字段1")
    private String reserve1;

    @Schema(description = "备用字段2")
    private String reserve2;

}
