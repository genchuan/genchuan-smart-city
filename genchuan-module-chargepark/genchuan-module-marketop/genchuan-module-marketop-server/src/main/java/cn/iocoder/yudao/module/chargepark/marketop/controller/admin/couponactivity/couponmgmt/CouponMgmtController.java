...
public class CouponMgmtController {
    ...
    public CommonResult<Boolean> importExcel(@RequestParam("file") MultipartFile file) throws Exception {
        ...
        log.info("解析后条数：{}", list == null ? "null" : list.size());
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                CouponMgmtImportExcelVO item = list.get(i);
                log.info("第{}条: name={}, type={}, amount={}", i, item.getName(), item.getType(), item.getAmount());
            }
        }
