...
@Schema(description = "管理后台 - 申诉复核 Response VO")
@Data
@ExcelIgnoreUnannotated
public class AppealRecordRespVO {

    @Schema(description = "主键ID", requiredMode = Schema.RequiredMode.REQUIRED, example = "14679")
    @ExcelProperty("主键ID")
    private Long id;

    @Schema(description = "申诉UUID（主键，UUID）", example = "29703")
    @ExcelProperty("申诉UUID（主键，UUID）")
    ...
    @ExcelProperty("申诉对象ID（关联eval_object.object_id）")
    ...
    @ExcelProperty("复核人员ID（关联sys_user.user_id，多个用逗号分隔）")
    ...
    @ExcelProperty("最终复核结果（维持原结果/修正结果）")
    ...
    @ExcelProperty("修正后标准ID（关联eval_standard_item.standard_item_id）")
    private String correctStandardId;

    @Schema(description = "结案状态（未结案/已结案）", example = "2")
    @ExcelProperty("结案状态（未结案/已结案）")
    private String closeStatus;

    @Schema(description = "结案人（关联sys_user.user_id）")
    @ExcelProperty("结案人（关联sys_user.user_id）")
