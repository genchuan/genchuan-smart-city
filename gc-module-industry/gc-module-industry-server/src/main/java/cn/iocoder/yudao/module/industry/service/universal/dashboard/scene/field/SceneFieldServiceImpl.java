...
public class SceneFieldServiceImpl implements SceneFieldService {
    ...
    public Long createSceneField(SceneFieldSaveReqVO createReqVO) {
        // 1.校验所属场景是否存在
        Long sceneId = Long.valueOf(createReqVO.getSceneId());
        boolean sceneExists = sceneFieldMapper.selectById(sceneId) != null;
        if (!sceneExists) {
            throw exception(new ErrorCode(400, "父级场景不存在"));
        }

        // 2.校验唯一字段标识是否唯一（同一场景下）
        boolean fieldIdExists = sceneFieldMapper.selectCount(
                new LambdaQueryWrapper<SceneFieldDO>()
                        .eq(SceneFieldDO::getSceneId, createReqVO.getSceneId())
                        .eq(SceneFieldDO::getFieldId, createReqVO.getFieldId())
    ...
    @Override
    public List<SceneFieldRespVO> listFiledBySceneId(Long sceneId) {
        // 1. 查询数据库实体列表
        List<SceneFieldDO> list = sceneFieldMapper.selectList(
                new LambdaQueryWrapper<SceneFieldDO>()
                        .eq(SceneFieldDO::getSceneId, sceneId)
                        .eq(SceneFieldDO::getDeleted, 0)
        );
