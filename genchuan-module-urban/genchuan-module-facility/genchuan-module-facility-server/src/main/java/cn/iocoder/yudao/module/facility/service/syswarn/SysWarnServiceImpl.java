...
//    }
    ...
    public PageResult<SysWarnRespVO> getSysWarnPage(SysWarnPageReqVO pageReqVO) {
        ...
        // 0. 查询总数
        Long total = sysWarnMapper.countSysWarnPage(pageReqVO);
        if (total == 0) {
    ...
    public Integer batchConfirmInvalid(SysWarnBatchConfirmValidReqVO reqVO) {
        ...
        // ================== 3. 查询预警状态 ==================
        List<SysWarnDO> warnList = sysWarnMapper.selectBatchIds(idList);

        if (warnList.size() ==0) {
            throw exception(400, "预警数据不存在，请重新选择");
        }

        // ================== 4. 校验状态必须为待处置 ==================
// 筛选出状态为“待处置”的预警
        List<SysWarnDO> pendingWarnList = warnList.stream()
                .filter(warn -> "待处置".equals(warn.getStatus()))
                .toList();

        if (pendingWarnList.isEmpty()) {
            throw exception(400, "只有【待处置】状态的预警才能归档,当前选择的数据无待处置数据");
        }
// 提取待处置预警ID
        List<Long> pendingWarnIdList = pendingWarnList.stream()
                .map(SysWarnDO::getId)
                .toList();

        //2.调用通用批量更新接口
        SysWarnBatchUpdateReqVO sysWarnBatchUpdateReqVO =new SysWarnBatchUpdateReqVO();
        sysWarnBatchUpdateReqVO.setIdList(pendingWarnIdList);
