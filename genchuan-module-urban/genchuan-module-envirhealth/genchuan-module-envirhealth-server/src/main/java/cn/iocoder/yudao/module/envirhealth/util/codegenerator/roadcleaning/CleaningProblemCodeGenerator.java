package cn.iocoder.yudao.module.envirhealth.util.codegenerator.roadcleaning;

import cn.iocoder.yudao.module.envirhealth.dal.mysql.roadcleaning.CleaningProblemMapper;
import jakarta.annotation.Resource;
import org.springframework.stereotype.Component;

/**
 * 功能:
 * 作者:SWE22008
 * 日期:2026/3/11 14:37
 */
@Component
public class CleaningProblemCodeGenerator {

    @Resource
    private CleaningProblemMapper cleaningProblemMapper;

    /**
     * 生成problem_id（格式：uuid-problem- + 3位序号，如 uuid-problem-001）
     */
    public String generateProblemId() {
        // 1. 查询全局最大序号（也可按日期维度，根据需求调整）
        Integer maxSeq = cleaningProblemMapper.selectMaxSeq();
        // 2. 新序号
        int newSeq = maxSeq == null ? 1 : maxSeq + 1;
        // 3. 拼接problem_id
        return String.format("uuid-problem-%03d", newSeq);
    }
}