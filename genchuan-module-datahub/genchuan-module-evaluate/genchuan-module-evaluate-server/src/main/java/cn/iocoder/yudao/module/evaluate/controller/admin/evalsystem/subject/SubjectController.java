...
public class SubjectController {
    ...
    public CommonResult<Integer> importSubjects(@RequestParam("file") MultipartFile file) throws IOException {
//        // 无有效数据直接返回
//        if (validVOList.isEmpty()) {
//            return CommonResult.success(0);
//        }
        // 1. 空文件校验（Controller仅做基础参数校验）
        if (file.isEmpty()) {
            return CommonResult.error(400, "导入文件不能为空");
        }
        // 2. 核心逻辑全部交给Service处理
        int importCount = subjectService.importSubjects(file.getInputStream());
        // 3. 仅返回结果，无任何业务逻辑
        return CommonResult.success(importCount);
    }
    // -------------------------- 新增联表查询接口 --------------------------
