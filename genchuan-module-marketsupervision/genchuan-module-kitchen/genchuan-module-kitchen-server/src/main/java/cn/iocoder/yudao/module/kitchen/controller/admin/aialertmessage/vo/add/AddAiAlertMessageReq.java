package cn.iocoder.yudao.module.kitchen.controller.admin.aialertmessage.vo.add;
...
public class AddAiAlertMessageReq {
    ...
    @Schema(description = "[消息来源] 如：1-端侧/2-云化/3-云侧/6-盒子",hidden = true)
    ...
    @Schema(description = "[消息版本] 消息版本",hidden = true)
    private String msgVersion = "1.0.0";

    @Schema(description = "[平台告警ID] 能力开放平台告警ID	（可能重复）", example = "6455",hidden = true)
    private Long alertId=6455L;

    @Schema(description = "[AI平台消息ID] AI平台唯一消息ID", example = "6397",hidden = true)
    private String aiPlatformMsgId = "6397";

    @Schema(description = "[检测框] 所有云侧都支持画框。这个字段默认存在，端侧（云化和端侧）不支持画框的能力为：" +
            "口罩识别、电动车识别、火情告警、区域入侵、设备巡检、人脸布控、车牌识别、静态客流统计、动态客流统计、客流统计（云眼专用）、车辆占道违停",hidden = true)
    private String bbox;

    @Schema(description = "[重复告警] 1:重复告警 0:非重复告警",hidden = true)
    private Integer repeatAlarm = 0;

    //TODO 随机60-300s
    @Schema(description = "[离岗时间] 作用：设置的离岗时间，超过这个时间触发离岗告警，单位（秒）", hidden = true)
    private Integer leaveTime;

    //TODO 随机
    @Schema(description = "[结束时间] 一天内的结束时间，格式为hh:mm，比如15:00",hidden = true)
    private String timeSlotEnd;

    //TODO 随机
    @Schema(description = "[间隔时间]",hidden = true)
    private Integer intervalTime;

    @Schema(description = "[完整告警参数JSON] 原始alertParams数据",hidden = true)
