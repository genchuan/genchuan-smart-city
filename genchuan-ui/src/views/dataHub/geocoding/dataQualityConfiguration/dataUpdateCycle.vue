    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          <el-form-item label="更新对象类型" prop="updateObjectType">
            <el-select v-model="searchForm.updateObjectType" placeholder="请选择更新对象类型" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.updateObjectTypeOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
          <el-form-item label="常规更新周期" prop="regularCycle">
            <el-select v-model="searchForm.regularCycle" placeholder="请选择常规更新周期" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.regularCycleOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
          <el-form-item label="特殊更新触发条件" prop="specialTriggerCondition">
            <el-input v-model="searchForm.specialTriggerCondition" placeholder="请输入特殊更新触发条件" clearable class="!w-240px" @change="searchClick"/>
          </el-form-item>
          <el-form-item label="更新责任部门" prop="updateDept">
            <el-tree-select v-model="searchForm.updateDept" class="!w-240px" :data="deptList" :props="defaultProps" check-strictly node-key="id" placeholder="请选择归属部门" @change="searchClick"/>
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column label="更新对象类型" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('更新对象类型')">
            <template #default="scope">
              <div @click="tableOneClick(scope.row)" class="table-active"> {{ handleTypeOptions(scope.row.updateObjectType, OptionsAll.updateObjectTypeOptions) }}</div>
            </template>
          </el-table-column>
          <el-table-column label="常规更新周期" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('常规更新周期')">
            <template #default="scope">
              {{ handleTypeOptions(scope.row.regularCycle, OptionsAll.regularCycleOptions) }}
            </template>
          </el-table-column>
          <el-table-column label="特殊更新触发条件" min-width="150" align="center" prop="specialTriggerCondition" :show-overflow-tooltip="true" v-if="tableColumnShow('特殊更新触发条件')"/>
          <el-table-column label="更新责任部门" min-width="150" align="center" prop="updateDept" :show-overflow-tooltip="true" v-if="tableColumnShow('更新责任部门')"/>
          <el-table-column label="更新流程说明" min-width="150" align="center" prop="updateProcessDesc" :show-overflow-tooltip="true" v-if="tableColumnShow('更新流程说明')"/>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['dataQualityConfiguration:dataUpdateCycle:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active==1">
              ...
              <div class="see-content" v-loading="drawerConfig.loading" v-if="drawerConfig.type === 'query'">
                <ul>
                  <li class="width50"><span>更新对象类型：</span>{{ handleTypeOptions(queryData.updateObjectType, OptionsAll.updateObjectTypeOptions) }}</li>
                  <li class="width50"><span>常规更新周期：</span>{{ handleTypeOptions(queryData.regularCycle, OptionsAll.regularCycleOptions) }}</li>
                  <li class="width50"><span>特殊更新触发条件：</span>{{ queryData.specialTriggerCondition }}</li>
                  <li class="width50"><span>更新责任部门：</span>{{ queryData.updateDept }}</li>
                  <li class="width100"><span>更新流程说明：</span>{{ queryData.updateProcessDesc }}</li>
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                <el-form :model="formData" ref="drawerFromRef" class="edit-content-form" label-width="140px">
                  <el-form-item label="更新对象类型：" prop="updateObjectType" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择更新对象类型" v-model="formData.updateObjectType" filterable clearable>
                      <el-option v-for="item in OptionsAll.updateObjectTypeOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="常规更新周期：" prop="regularCycle" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择常规更新周期" v-model="formData.regularCycle" filterable clearable>
                      <el-option v-for="item in OptionsAll.regularCycleOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="特殊更新触发条件：" prop="specialTriggerCondition" class="edit-content-form-li width50" :rules="[]">
                    <el-input placeholder="请输入特殊更新触发条件" v-model="formData.specialTriggerCondition" clearable/>
                  </el-form-item>
                  <el-form-item label="更新责任部门" prop="updateDept" class="edit-content-form-li width50" :rules="[]">
                    <el-tree-select v-model="formData.updateDept" :data="deptList" :props="defaultProps" check-strictly node-key="id" placeholder="请选择更新责任部门"/>
                  </el-form-item>
                  <el-form-item label="更新流程说明：" prop="updateProcessDesc" class="edit-content-form-li width100" :rules="[]">
                    <el-input placeholder="请输入更新流程说明" v-model="formData.updateProcessDesc" type="textarea" :autosize="{ minRows: 2, maxRows: 6}" clearable/>
                  </el-form-item>
