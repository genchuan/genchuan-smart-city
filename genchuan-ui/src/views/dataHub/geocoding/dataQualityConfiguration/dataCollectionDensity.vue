    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          <el-form-item label="区域类别" prop="regionCategory">
            <el-select v-model="searchForm.regionCategory" placeholder="请选择区域类别" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.regionCategoryOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
          <el-form-item label="区域类别说明" prop="regionCategoryDesc">
            <el-select v-model="searchForm.regionCategoryDesc" placeholder="请选择区域类别说明" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.regionCategoryDescOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column label="区域类别" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('区域类别')">
            <template #default="scope">
              <div @click="tableOneClick(scope.row)" class="table-active"> {{ handleTypeOptions(scope.row.regionCategory, OptionsAll.regionCategoryOptions) }}</div>
            </template>
          </el-table-column>
          <el-table-column label="区域类别说明" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('区域类别说明')">
            <template #default="scope">
              {{ handleTypeOptions(scope.row.regionCategoryDesc, OptionsAll.regionCategoryDescOptions) }}
            </template>
          </el-table-column>
          <el-table-column label="特殊兴趣点采集要求" min-width="150" align="center" prop="specialPoiRequirement" :show-overflow-tooltip="true" v-if="tableColumnShow('特殊兴趣点采集要求')"/>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['dataQualityConfiguration:dataCollectionDensity:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['dataQualityConfiguration:dataCollectionDensity:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['dataQualityConfiguration:dataCollectionDensity:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active==1">
              ...
              <div class="see-content" v-loading="drawerConfig.loading" v-if="drawerConfig.type === 'query'">
                <ul>
                  <li class="width50"><span>区域类别：</span>{{ handleTypeOptions(queryData.regionCategory, OptionsAll.regionCategoryOptions) }}</li>
                  <li class="width50"><span>区域类别说明：</span>{{ handleTypeOptions(queryData.regionCategoryDesc, OptionsAll.regionCategoryDescOptions) }}</li>
                  <li class="width100"><span>特殊兴趣点采集要求：</span>{{ queryData.specialPoiRequirement }}</li>
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                <el-form :model="formData" ref="drawerFromRef" class="edit-content-form" label-width="140px">
                  <el-form-item label="区域类别：" prop="regionCategory" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择区域类别" v-model="formData.regionCategory" filterable clearable>
                      <el-option v-for="item in OptionsAll.regionCategoryOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="区域类别说明：" prop="regionCategoryDesc" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择区域类别说明" v-model="formData.regionCategoryDesc" filterable clearable>
                      <el-option v-for="item in OptionsAll.regionCategoryDescOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="特殊兴趣点采集要求：" prop="specialPoiRequirement" class="edit-content-form-li width100" :rules="[]">
                    <el-input placeholder="请输入特殊兴趣点采集要求" v-model="formData.specialPoiRequirement" type="textarea" :autosize="{ minRows: 2, maxRows: 6}" clearable/>
                  </el-form-item>
