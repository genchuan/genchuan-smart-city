    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          <el-form-item label="地形类型" prop="terrainType">
            <el-select v-model="searchForm.terrainType" placeholder="请选择地形类型" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.terrainTypeOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
          <el-form-item label="适用数据类型" prop="applicableDataType">
            <el-select v-model="searchForm.applicableDataType" placeholder="请选择适用数据类型" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.applicableDataTypeOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column label="地形类型" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('地形类型')">
            <template #default="scope">
              <div @click="tableOneClick(scope.row)" class="table-active"> {{ handleTypeOptions(scope.row.terrainType, OptionsAll.terrainTypeOptions) }}</div>
            </template>
          </el-table-column>
          <el-table-column label="适用数据类型" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('适用数据类型')">
            <template #default="scope">
              {{ handleTypeOptions(scope.row.applicableDataType, OptionsAll.applicableDataTypeOptions) }}
            </template>
          </el-table-column>
          <el-table-column label="比例尺说明" min-width="150" align="center" prop="scaleDesc" :show-overflow-tooltip="true" v-if="tableColumnShow('比例尺说明')"/>
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['dataQualityConfiguration:positionAccuracyStandard:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active==1">
              ...
              <div class="see-content" v-loading="drawerConfig.loading" v-if="drawerConfig.type === 'query'">
                <ul>
                  <li class="width50"><span>地形类型：</span>{{ handleTypeOptions(queryData.terrainType, OptionsAll.terrainTypeOptions) }}</li>
                  <li class="width50"><span>适用数据类型：</span>{{ handleTypeOptions(queryData.applicableDataType, OptionsAll.applicableDataTypeOptions) }}</li>
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                <el-form :model="formData" ref="drawerFromRef" class="edit-content-form" label-width="140px">
                  <el-form-item label="地形类型：" prop="terrainType" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择地形类型" v-model="formData.terrainType" filterable clearable>
                      <el-option v-for="item in OptionsAll.terrainTypeOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="适用数据类型：" prop="applicableDataType" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择适用数据类型" v-model="formData.applicableDataType" filterable clearable>
                      <el-option v-for="item in OptionsAll.applicableDataTypeOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="比例尺说明：" prop="scaleDesc" class="edit-content-form-li width100" :rules="[]">
                    <el-input placeholder="请输入比例尺说明" v-model="formData.scaleDesc" type="textarea" :autosize="{ minRows: 2, maxRows: 6}" clearable/>
                  </el-form-item>
