    ...
    <div class="cc-main-content">
      ...
      <div class="cc-search-css" ref="searchRef">
        ...
        >
          <el-form-item label="定位对象类型" prop="locationType">
            <el-select v-model="searchForm.locationType" placeholder="请选择定位对象类型" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.locationTypeOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
          <el-form-item label="分段规则" prop="segmentRule">
            <el-input v-model="searchForm.segmentRule" placeholder="请输入分段规则" clearable class="!w-240px" @change="searchClick"/>
          </el-form-item>
          <el-form-item label="组合规则" prop="combineRule">
            <el-input v-model="searchForm.combineRule" placeholder="请输入组合规则" clearable class="!w-240px" @change="searchClick"/>
          </el-form-item>
          <el-form-item label="方位" prop="directionOptions">
            <el-select v-model="searchForm.directionOptions" placeholder="请选择方位" class="!w-240px" clearable @change="searchClick">
              <el-option v-for="(item,key) in OptionsAll.directionOptionsOptions" :key="key" :label="item.label" :value="item.value"/>
            </el-select>
          </el-form-item>
      ...
      <div class="cc-table-css">
        ...
        >
          ...
          <el-table-column label="定位对象类型" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('定位对象类型')">
            <template #default="scope">
              <div @click="tableOneClick(scope.row)" class="table-active"> {{ handleTypeOptions(scope.row.locationType, OptionsAll.locationTypeOptions) }}</div>
            </template>
          </el-table-column>
          <el-table-column label="分段规则" min-width="150" align="center" prop="segmentRule" :show-overflow-tooltip="true" v-if="tableColumnShow('分段规则')"/>
          <el-table-column label="组合规则" min-width="150" align="center" prop="combineRule" :show-overflow-tooltip="true" v-if="tableColumnShow('组合规则')"/>
          <el-table-column label="方位" min-width="150" align="center" :show-overflow-tooltip="true" v-if="tableColumnShow('方位')">
            <template #default="scope">
          ...
          <el-table-column width="300" fixed="right" label="操作" align="center">
            <template #default="scope">
              <el-button type="success" :icon="View" :plain="true" @click="queryClick(scope.row)" v-hasPermi="['codingRules:geocodedSegmentationCombinationRules:query']">详细</el-button>
              <el-button type="primary" :icon="EditPen" :plain="true" @click="editClick(scope.row)" v-hasPermi="['codingRules:geocodedSegmentationCombinationRules:update']">编辑</el-button>
              <el-button type="danger" :icon="Delete" :plain="true" @click="delClick(scope.row)" v-hasPermi="['codingRules:geocodedSegmentationCombinationRules:delete']">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
          ...
          <div class="drawer-content">
            ...
            <template v-if="drawerConfig.active==1">
              ...
              <div class="see-content" v-loading="drawerConfig.loading" v-if="drawerConfig.type === 'query'">
                <ul>
                  <li class="width50"><span>定位对象类型：</span>{{ handleTypeOptions(queryData.locationType, OptionsAll.locationTypeOptions) }}</li>
                  <li class="width50"><span>分段规则：</span>{{ queryData.segmentRule }}</li>
                  <li class="width50"><span>组合规则：</span>{{ queryData.combineRule }}</li>
                  <li class="width50"><span>方位：</span>{{ handleTypeOptions(queryData.directionOptions, OptionsAll.directionOptionsOptions) }}</li>
              ...
              <div class="edit-content" v-loading="drawerConfig.loading" v-else>
                <el-form :model="formData" ref="drawerFromRef" class="edit-content-form" label-width="140px">
                  <el-form-item label="定位对象类型：" prop="locationType" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择定位对象类型" v-model="formData.locationType" filterable clearable>
                      <el-option v-for="item in OptionsAll.locationTypeOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
                  <el-form-item label="分段规则：" prop="segmentRule" class="edit-content-form-li width50" :rules="[]">
                    <el-input placeholder="请输入分段规则" v-model="formData.segmentRule" clearable/>
                  </el-form-item>
                  <el-form-item label="组合规则：" prop="combineRule" class="edit-content-form-li width50" :rules="[]">
                    <el-input placeholder="请输入组合规则" v-model="formData.combineRule" clearable/>
                  </el-form-item>
                  <el-form-item label="方位：" prop="directionOptions" class="edit-content-form-li width50" :rules="[]">
                    <el-select placeholder="请选择方位" v-model="formData.directionOptions" filterable clearable>
                      <el-option v-for="item in OptionsAll.directionOptionsOptions" :key="item.value" :label="item.label" :value="item.value"/>
                    </el-select>
                  </el-form-item>
