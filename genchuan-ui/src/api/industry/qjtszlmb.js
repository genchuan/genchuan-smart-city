...
export const fetchRegionDict = () => {
  try {
    return request.get({
      ...
      console.log('行政区域字典接口调用失败-兜底模拟数据:', error.message);
...
export const fetchUrbanManagementCoreIndicators = (params = {}) => {
  try {
    return request.get({
      ...
      console.log('城管住建核心指标接口调用失败-兜底模拟数据:', error.message);
...
export const fetchUrbanConstructionGeometries = (params = {}) => {
  try {
    return request.get({
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          console.log('城管住建接口调用失败-使用模拟数据兜底，筛选参数：', params);
          const mockData = [
            {
              road_fac_id: "QZ001",
