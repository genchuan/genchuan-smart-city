...
public class FundSystemController {
    ...
    public void exportFundSystemExcel(@Valid FundSystemPageReqVO pageReqVO,
        ...
        CommonResult<List<DictDataRespDTO>> typeDictDataList = dictDataApi.getDictDataList(StudentMgmtDictTypeEnum.FUND_SYSTEM_FUND_TYPE.getType());
        CommonResult<List<DictDataRespDTO>> statusDictDataList = dictDataApi.getDictDataList(StudentMgmtDictTypeEnum.FUND_SYSTEM_STATUS.getType());
        list = list.stream().map(item -> {
            ...
            if (typeDictDataList.getData() != null) {
                for (DictDataRespDTO dictData : typeDictDataList.getData()) {
                    if (dictData.getValue().equals(fundType)) {
                        fundType = dictData.getLabel();
                        break;
                    }
                }
            }
            item.setFundType(fundType);
